...
public class FilterTest {
    ...
    /*
     * Partition on tweets.length(): zero, one, more than one. Partition on number
     * of tweets made by author <username>: zero, one, more than one.
     */

    /*
     * Tests that cover cartesian product of subdomains: 1. single item, one tweet
     * from author<username>. 2. more than one item, zero tweet made by
     * author<username>. 3. more than one item, one tweet made by author <username>.
     * 4. more than one item, more than one tweets made by author <username>. 5.
     * empty list and zero tweets from author.
     */

    // Testing strategy for inTimeSpan
    /*
     * Partition on tweets.length():zero, one item, more than one item. Partition on
     * value of timespan.start: T.start <= tweets[min].timestamp,
     ...
     * tweets[min].timestamp < T.end < tweets[max].timestamp, tweets[max].timestamp
     * <= T.end
     * 
     * Partition on number of tweets within the timestamp: zero, one, 1 < x <
     * tweets.length(), tweets.length()
     * 
     * Partition on number of tweets that overlap with timespan.start:zero, one,
     * more than one
     * 
     * Partition on number of tweets that overlap timespan.end:zero, one, one more
     * than one
     */
...
// 	   tweets.length() tweets within timespan,and more than one tweet overlap with T.start and T.end
     ...
     * Partition on number of words: one, more than one. Partition on number of
     * tweets with atleast one word from words: zero, one, 1 < x < tweets.length(),
     * tweets.length(),
     */
...
//     6. zero tweet, one word, zero tweet with atleast one word from words,
    ...
    private static final Tweet tweet5 = new Tweet(5, "rose",
	    "@eli I will email you the details.My mail id is rose@newage.com.", d5);
    private static final Tweet tweet6 = new Tweet(6, "alyssa", "@rose @eli @dappy This was a big revelation for me.",
	    d6);
...
//  4. more than one item, more than one tweet made by author <username>,
	...
	List<Tweet> writtenBy = Filter.writtenBy(Arrays.asList(tweet1, tweet2, tweet4, tweet5, tweet6, tweet9),
		"alyssa");
...
//	5. empty list and zero tweets from the author.
    ...
    // 1. more than one item, T.start = tweets[min].timestamp, tweets[max].timestamp
    // = T.end, tweets.length() tweets within timespan, more than one tweet overlap
    // with T.start , one tweet overlaps with T.end
    @Test
	...
	assertTrue("expected list to contain tweets",
		inTimespan.containsAll(Arrays.asList(tweet1, tweet2, tweet3, tweet4, tweet5, tweet6, tweet7)));
...
//	4. more than one item, tweets[min].timestamp <= T.start <= tweets[max].timestamp, tweets[min].timestamp <= T.end <= tweets[max].timestamp, 1 < x < tweets.length() tweets with in timespan,zero tweet overlap with T.start and T.end
	...
	List<Tweet> inTimespan = Filter.inTimespan(
		Arrays.asList(tweet4, tweet5, tweet6, tweet7, tweet8, tweet9, tweet10),
		new Timespan(testStart, testEnd));

	assertFalse("expected non-empty list", inTimespan.isEmpty());
	assertTrue("expected list to contain tweets",
		inTimespan.containsAll(Arrays.asList(tweet5, tweet6, tweet7, tweet8)));
...
//	    tweets.length() tweets within timespan,and more than one tweet overlap with T.start and T.end
	...
	assertFalse("expected non-empty list", inTimespan.isEmpty());
	assertTrue("expected list to contain tweets", inTimespan.containsAll(
		Arrays.asList(tweet1, tweet2, tweet3, tweet4, tweet5, tweet6, tweet7, tweet8, tweet9, tweet10)));
	assertEquals("expected same order", 0, inTimespan.indexOf(tweet1));
	assertEquals("expected same order", 1, inTimespan.indexOf(tweet2));
	assertEquals("expected same order", 2, inTimespan.indexOf(tweet3));
...
//  3. more than one item, more than one word, all tweets have atleast one word from word, a single word matches multiple tweets.
	...
	List<Tweet> containing = Filter.containing(Arrays.asList(tweet2, tweet3, tweet4, tweet5),
		Arrays.asList("I", "hate"));

	assertTrue("expected list to contain tweets",
		containing.containsAll(Arrays.asList(tweet2, tweet3, tweet4, tweet5)));
...
//  4. more than one item, one word,1 < x < tweets.length() tweets with atleast one word from words, a single word matches multiple tweets.
	...
	List<Tweet> containing = Filter.containing(Arrays.asList(tweet5, tweet6, tweet7, tweet8, tweet9, tweet10),
		Arrays.asList("have"));
